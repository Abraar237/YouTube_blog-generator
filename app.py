...
def summarize(result):
    ...
    sys_instruct = (
        f"You are a YouTube Notes Maker who creates notes or docs based on the transcript provided. "
        f"Use only the transcript to make notes in {language} language, highlight the main topics, "
        f"and structure it with subheadings. Don't mention anything else except an explanation in your response. "
        f"Just mention the video title: {result.get('title')} and thumbnail URL: {result.get('thumbnail_url')} at the top. "
        f"Write in more detail with at least 3-4 subtitles, ensuring important topics are covered. "
        f"Subheadings can also be questions highlighting key points. If the transcript is small, "
        f"make sure to cover most of the topics in the summary. Always include a summary at the end."
    )
    client = genai.Client(api_key=st.secrets["K"])
...
<body class="min-h-screen flex items-center justify-center p-4">
  <div class="max-w-2xl w-full bg-white rounded-2xl shadow-lg p-6">
    ...
    '''
    client1 = genai.Client(api_key=st.secrets["K"])
